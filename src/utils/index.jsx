...
export function checkDuplicate(item, allItems) {
    const nameDuplicated = allItems.find(ele => (ele.name === item.name));
    const idDuplicated = allItems.find(ele => (ele._id === item._id));
...
export function getListStyle(isDraggingOver, isOwn, isFull) {
    let color = 'white';
    if (isDraggingOver) {
        if (!isOwn && isFull) { color = 'lightcoral'; } else { color = 'lightblue'; }
    }
    return {
        background: color,
        padding: 0,
